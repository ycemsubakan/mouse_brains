...
vis = visdom.Visdom(port=0, server='', env=env)
...
#nbins = 500
#hist = torch.histc(data, nbins) 
#
#plt.semilogx(np.linspace(data.min(), data.max(), nbins), hist.numpy(), '-o')
#plt.xlabel('pixel_value')
#plt.ylabel('count')
#
#plt.savefig('mice_hist.png', format='png')

#train_data_dir = '/your_root/data2/crop_celeba_train/'          # this path depends on your directories
#test_data_dir = '/your_root/data2/crop_celeba_test/'

#dset_train = datasets.ImageFolder(train_data_dir, transform=transform)
#train_loader = torch.utils.data.DataLoader(dset_train, batch_size=12, shuffle=True,
#                                           pin_memory=True, num_workers=arguments.num_gpus)
...
if model == 'NF':
    ...
    for config_num, Ks in enumerate(Kss):
        ...
        path = 'models/VAE_arc3_l1_leakyrelu_{}_K_{}.t'.format(arguments.data, Ks)
